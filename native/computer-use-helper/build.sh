...
# Brand the helper with its OWN "BlitzOS Automation" mark (the obsidian gear icon) so the Accessibility /
# Screen Recording / Automation rows + the drag tile show the EXACT icon the user is granting, not the main
# BlitzOS loop or a generic executable icon. AppIcon.png sits next to this script (corners pre-masked to a
# transparent squircle); AppIcon.icns is generated from it below.
