...
const api = {
  ...
  /** Best-effort local user identity (username + host) for the support chat's "who is writing" subject. */
  whoami(): Promise<{ user: string; host: string }> {
    return (ipcRenderer.invoke('os:whoami') as Promise<{ user: string; host: string }>).catch(() => ({ user: '', host: '' }))
  },
  ...
  /** The FULL raw jsonl transcript of an agent session, gzipped+base64, for "attach this session" to support. Null if none. */
  agentTranscript(id: string): Promise<{ gzipB64: string; bytes: number; tooLarge?: boolean } | null> {
    return (ipcRenderer.invoke('os:agent-transcript', { id }) as Promise<{ gzipB64: string; bytes: number; tooLarge?: boolean } | null>).catch(() => null)
  },
