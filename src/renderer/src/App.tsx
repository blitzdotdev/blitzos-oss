...
export default function App(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent): void => {
      ...
      // Pure bounding-box test — independent of hit-testing, so it still works when the chassis is pointer-events:none
      // (the VEIL escape below needs this: a veiled chassis is pointer-events:none, so elementFromPoint skips it).
      ...
      // VEIL escape hatch independent of the hit-window (a no-notch display has none, so onHandleHover never fires):
      // if the island is veiled and the cursor lands on the laid-out-but-invisible chassis, un-veil it here, mirroring
      // the hit-window hover un-veil. Uses inPanelRect (pure bbox), NOT inPanel, because the veiled chassis is
      // pointer-events:none so elementFromPoint can't hit it. MUST run before setNotchInteractive (the veil chokepoint
      // forces interactive off while veiled).
      if (notchVeiledRef.current && inPanelRect) {
        notchVeiledRef.current = false
        document.body.classList.remove('island-veiled')
      }
