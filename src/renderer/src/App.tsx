...
export default function App(): JSX.Element {
  ...
  // PINNED panel: a KEYBOARD-opened panel (⌥Space) or a menu-opened Settings panel stays open regardless of the mouse,
  // UNTIL the user does a deliberate enter+leave of the ISLAND — then it reverts to hover-follow. This lifecycle is
  // driven by the mousemove handler's `want` (cursor over the island, INCLUDING the notch via notchOverRef), NOT by
  // the chassis pointer events: the notch hit-window (a real OS window over the top-center) occludes the chassis, so
  // a horizontal sweep across the top fired spurious chassis leave→enter that cleared the pin (the "any X movement
  // closes it" bug). `want` has no such seam. Grant-flow / synthetic (VM) pins are separate and survive a leave.
  const [notchPinned, setNotchPinned] = useState(false)
  const notchPinnedRef = useRef(false)
  // The enter→leave state machine: pinArmed = the cursor has been OUTSIDE the island since the pin was set (so a later
  // inside counts as a deliberate enter, NOT the island merely opening under the cursor). pinEngaged = a deliberate
  // enter has happened; the next time the cursor is outside, release the pin (and let it hover-close).
  ...
  const setNotchPinnedBoth = (on: boolean): void => {
    ...
    setNotchPinned(on)
    if (on) {
      pinArmedRef.current = false // a fresh pin starts the enter→leave cycle: not yet been outside, not yet engaged.
  ...
  // can't open the island. It starts pinned OPEN + interactive; ⌥Space is the SOLE show/hide toggle (toggleIsland),
  // since hover/hit-window can't reopen it in a VM. Hover-retract and Esc / os:notch-close (closeIsland) stay no-ops,
  // so ONLY ⌥Space ever hides it and it can never get stranded shut by anything else.
  ...
  const setChassisHover = (on: boolean): void => {
    ...
    // NOTE: the ⌥Space/menu PIN lifecycle is NOT handled here — chassis pointer enter/leave is corrupted by the notch
    // hit-window occluder (a horizontal sweep across the top fires false leave→enter). It lives in the mousemove
    // handler, keyed on `want` (which has no such seam). This handler only does the hover keep-open / close.
    if (on) {
  ...
  const toggleIsland = (): void => {
    ...
    if (notchStateRef.current === 'closed') {
      setNotchPinnedBoth(true) // a keyboard-opened panel stays open regardless of the mouse (until a real enter→leave)
      setNotchInteractive(true)
      applyNotchState('panel') // opens to the LAST view+tab (islandViewRef/islandPageRef), not a forced new session
    } else {
      // hide (panel → closed). In VM/synthetic mode ⌥Space is the SOLE show/hide toggle, so this runs there too:
      // ⌥Space-open reopens it (hover/hit-window can't in a VM). Esc / os:notch-close stay no-ops (closeIsland guard).
      setNotchPinnedBoth(false)
      setNotchAnimating(true) // freeze widget motion during the collapse (smooth)
      applyNotchState('closed')
      notchLastIRef.current = null
      setNotchInteractive(false)
      // ⌥Space-hide is an explicit dismiss → tear down a kept-mounted app preview (mirror closeIsland) so the
      // next open remounts fresh + restores the last view+tab instead of the stale app iframe.
  ...
  useEffect(
    () =>
      window.agentOS?.notch?.onGeometry?.((g) => {
        ...
        setNotchOn(true)
        // A display change to a NO-NOTCH display destroys the hit-window with no DOM mouseleave, so notchOverRef
        // (set true ONLY by the hit-window's hover) can strand true. Stuck true → the mousemove handler forces the
        // overlay interactive on every move (eats clicks across all of macOS) AND re-opens the island, unrecoverable
        // without a restart. With no notch the hit-window can't be hovered, so any lingering flag is stale: clear it.
        if (!g.hasNotch) notchOverRef.current = false
  ...
  const closeIsland = (): void => {
    ...
    notchLastIRef.current = null
    setNotchInteractive(false)
    // EXPLICIT dismiss (Esc / os:notch-close) tears down a kept-mounted app preview, so NotchHost actually
    // unmounts and the next open remounts fresh + restores the last view+tab (instead of re-showing the stale
    // app iframe). Hover-away does NOT do this — it keeps the preview warm (see scheduleNotchHoverClose).
    islandActiveAppRef.current = null
    setIslandKeepMounted(false)
  }
  ...
  const openIslandSettings = (): void => {
    ...
    if (notchStateRef.current === 'closed') {
      // Pin so the menu-opened panel doesn't instantly hover-retract (the cursor is up in the menu bar, off the
      // island). Like ⌥Space, this pin only HOLDS until the cursor does a deliberate enter→leave of the island, then
      // reverts to hover-follow (the pin lifecycle in the mousemove handler, keyed on `want`), so Settings hover-closes
      // once you've moused in and back out — intended.
      setNotchPinnedBoth(true)
      setNotchInteractive(true)
      applyNotchState('panel')
    } else {
  ...
  useEffect(
    () =>
      window.agentOS?.notch?.onVeil?.((on) => {
        ...
        // On UNVEIL re-assert interactivity for any OPEN island, not just VM/synthetic: a grant that lands while the
        // cursor is off the island (e.g. in System Settings) otherwise leaves the overlay click-through until the next
        // mousemove — one dead click. 'panel' subsumes synthetic (which is always pinned panel).
        else if (notchStateRef.current === 'panel') setNotchInteractive(true)
      }),
    []
  )
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent): void => {
      ...
      // The shown panel is the NotchHost portal (the .nh-chassis shell). Match its VISIBLE pill: elementFromPoint
      // respects the chassis's rounded-bottom shape AND ignores the box-shadow GLOW (a shadow has no hit area), so
      // the hover/interactive region HUGS the visible edge. The old bounding-rect + 10px slop kept the island open /
      // capturing clicks ~10px out into the glow — the reported halo. Fall back to the BARE bounding rect (no slop)
      // ONLY when elementFromPoint can't resolve a node (cursor off the viewport); the hover-close is debounced, so a
      // one-frame DOM miss never snaps the island shut.
      const hitEl = document.elementFromPoint(e.clientX, e.clientY) as HTMLElement | null
      const inPanelDom = !!hitEl?.closest?.('.nh-chassis')
      const pr = (document.querySelector('.nh-chassis') as HTMLElement | null)?.getBoundingClientRect()
      // Pure bounding-box fallback for inPanel, used only when elementFromPoint can't resolve a node (cursor off-viewport).
      const inPanelRect = !!pr && e.clientX >= pr.left && e.clientX <= pr.right && e.clientY >= pr.top && e.clientY <= pr.bottom
      // inPanel hugs the VISIBLE pill: elementFromPoint respects the rounded shape + excludes the box-shadow glow. Bare
      // bbox is the fallback ONLY when elementFromPoint can't resolve a node (cursor off-viewport). Drives want/keep-open.
      const inPanel = inPanelDom || (!hitEl && inPanelRect)
      overChassisRef.current = inPanel
      // NO chassis-hover veil un-veil here. It broke the onboarding GRANT flow: grantActiveStep veils the island so the
      // macOS permission popup is the focus, but the user's cursor is still over the card (the chassis) when they click
      // Grant, so a chassis-based un-veil fired on the next mousemove and instantly un-veiled the island over the popup.
      // The veil's escape is the deliberate NOTCH hover (onHandleHover above) plus the grant flow's own grant-changed +
      // 30s safety timer; a no-notch display recovers via those + ⌥Space/Esc (the LOW gap from audit finding #4).
      ...
      // ⌥Space / menu PIN lifecycle, keyed on `want` so the notch hit-window seam (which breaks chassis pointer
      // enter/leave) can't trip it: the pin holds the island open until the cursor goes OUTSIDE (arm), then INSIDE
      // (a deliberate enter — NOT the island merely opening under the cursor), then OUTSIDE again → release the pin,
      // and the close path below then hover-closes it. Excludes the grant-flow + synthetic pins (they survive a leave).
      if (st === 'panel' && notchPinnedRef.current && !grantPinnedRef.current && !syntheticRef.current) {
        if (!want) {
          if (pinEngagedRef.current) setNotchPinnedBoth(false) // left after a deliberate enter → free the lock
          else pinArmedRef.current = true // been outside → a later enter now counts as deliberate
        } else if (pinArmedRef.current) {
          pinEngagedRef.current = true // outside→inside after arming = the deliberate enter
        }
      }
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent): void => {
      ...
      if (inZone) {
        ...
        notchHoldUntilRef.current = performance.now() + NOTCH_HOVER_OPEN_GRACE_MS
        // Keep the island OPEN while in the band, but do NOT force the overlay interactive on bare inZone — that made
        // the full-display overlay swallow menu-bar / menu-bar-app clicks across the whole ~600px top band. Re-assert
        // interactive ONLY over real island pixels (the chassis or a visible glance bar); off them, leave the
        // click-through decision to the notch-hover handler (it already set want-based interactivity this frame).
        const overBar =
          overChassisRef.current ||
          ['.glance-left', '.glance-right'].some((sel) => {
            const br = document.querySelector(sel)?.getBoundingClientRect()
            return !!br && e.clientX >= br.left - 4 && e.clientX <= br.right + 4 && e.clientY >= br.top && e.clientY <= br.bottom + 6
          })
        if (overBar) setNotchInteractive(true)
      } else {
        scheduleNotchHoverClose()
      }
    }
