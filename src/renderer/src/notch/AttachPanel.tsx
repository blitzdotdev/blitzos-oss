...
// The browser connection mini-onboarding ("Let BlitzOS Automation work in Chrome/Safari") lives in a MODULE store
...
export function AttachPanel({ activeSessionId = '' }: { activeSessionId?: string }): JSX.Element {
  ...
  // A browser-drop suspends the picker (setPickSuspended true) and relies on a later `connected` event to clear it.
  // That event reaches THIS effect's listener, which is gone if the panel unmounts in the ~1-2s window (e.g. the user
  // opens Settings) → pickSuspended strands true → the grant-pin sticks → hover-close dead + picker disarmed. This
  // token guards a self-healing clear (timeout + unmount cleanup) that fires ONLY while the drop is still unresolved,
  // so it never clobbers a suspend already handed off to the grant onboarding / pendingGrant.
  const dropSuspendPendingRef = useRef(false)
  ...
  useEffect(() => {
    const off = window.agentOS?.pick?.onEvent?.((m) => {
      if (m.kind === 'pick_over') setDragOver(!!m.inside)
        ...
        if (isBrowserApp(m.app as string | undefined)) {
          ...
          dropSuspendPendingRef.current = true
          // Self-heal if the clearing `connected` event is lost (panel unmounted before it arrived): release the
          // suspend after a safety window, but ONLY while still pending — a real connected/grant-handoff flips the
          // token false first, so this never releases a suspend now owned by the grant onboarding / pendingGrant.
          window.setTimeout(() => {
            if (dropSuspendPendingRef.current) {
              dropSuspendPendingRef.current = false
        ...
        dropSuspendPendingRef.current = false // the drop resolved (success / fail / grant-handoff) → disarm the safety net
        setDragOver(false)
        const wid = Number(m.windowId)
        ...
        if (!m.ok) {
          ...
          if (perm && perm.grant) {
            // A BROWSER grant → the dedicated "Let BlitzOS Automation work in {Chrome|Safari}" mini-onboarding (Chrome = 3 steps,
    ...
    return () => {
      ...
      // If the panel unmounts while a browser-drop still awaits its `connected` event, that event is lost — release
      // the drop's picker-suspend here so pickSuspended can't strand true (the grant-pin / hover-dead bug).
      if (dropSuspendPendingRef.current) {
        dropSuspendPendingRef.current = false
        setPickSuspended(false)
      }
    ...
    <div className="att">
      {browserOnboard ? (
        <div className="att-onboard">
          <span className="att-onboard-title">Let BlitzOS Automation work in {browserLabel(browserOnboard.browser)}</span>
          <span className="att-onboard-sub">
            {browserOnboard.steps.length} quick permission{browserOnboard.steps.length === 1 ? '' : 's'} so BlitzOS Automation can drive your tabs
          </span>
