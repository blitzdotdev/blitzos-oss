...
import IslandSupport from './IslandSupport'
import { useAttachMode, useAttachedSessions, exitAttach } from './attachStore'
...
export function NotchHost({
  ...
  const attachMode = useAttachMode()
  const attachCount = useAttachedSessions().size
  ...
  // Attach MODE (attachStore) is module-level and survives the remount, so closing the island mid-attach would leave
  // it stuck on for the next open. Reset it whenever the island goes hidden (keyed on `visible`, NOT mount-only — a
  // parked/warm app keeps NotchHost mounted). exitAttach keeps the selected set, so queued cards survive; only the
  // modal mode clears. No-op when attach mode is already off.
  useEffect(() => {
    if (!visible) exitAttach()
  }, [visible])
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent): void => {
      if (!visible) return // PARKED (kept mounted for a warm app preview while the island is hidden) → never page tabs of an invisible island
  ...
  const dataView =
    view === 'settings' ? 'settings' : view === 'support' ? 'settings' : view === 'onboarding' ? 'onboarding' : safePage === 0 ? 'session' : 'process'
      ...
      >
        {view === 'support' ? (
          <IslandSupport
            ...
            onClose={() => {
              holdChassisHover()
              setView('session') // back to the chat tab the user was on
            }}
          />
        ) : view === 'settings' ? (
          <IslandSettings
        ...
        {view === 'session' && visible && attachMode && !appViewerOpen && (
          <button
            type="button"
            className="nh-feedback nh-attach-done"
            style={{ top: Math.max(2, (menuBarH - 24) / 2) }}
            onClick={() => {
              exitAttach()
              holdChassisHover()
              setView('support')
            }}
            title="Done attaching — back to support"
            aria-label="Done attaching — back to support"
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" aria-hidden focusable="false">
              <path d="M5 12.5l4.2 4.2L19 7" />
            </svg>
            <span className="nh-feedback-label">{attachCount === 0 ? 'Done' : `Attach ${attachCount}`}</span>
          </button>
        )}
        {view === 'session' && visible && !attachMode && !appViewerOpen && (
          <button
            type="button"
            className="nh-feedback"
            style={{ top: Math.max(2, (menuBarH - 24) / 2) }} // sit at the macOS menu-bar-icon level, above the tab rail
            onClick={() => {
              holdChassisHover()
              setView('support')
            }}
            title="Send feedback to the BlitzOS team"
            aria-label="Send feedback to the BlitzOS team"
          >
            {/* SF Symbol bubble.left.and.text.bubble.right, drawn as SVG (SF glyphs don't render reliably in Chromium) */}
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden focusable="false">
              <path d="M2.5 4.5a2 2 0 0 1 2-2h7a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2H7l-2.5 2v-2a2 2 0 0 1-2-2z" />
              <path d="M21.5 13.5a2 2 0 0 0-2-2h-7a2 2 0 0 0-2 2v3a2 2 0 0 0 2 2h4.5l2.5 2v-2a2 2 0 0 0 2-2z" />
              <path d="M13.5 14h5M13.5 16h3" />
            </svg>
            <span className="nh-feedback-label">Feedback</span>
          </button>
        )}
      </div>
    </div>
