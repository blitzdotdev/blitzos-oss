...
import { useAttachMode, useAttachedSessions, toggleSession } from './attachStore'
...
export default function IslandPanel(props: IslandPanelProps): JSX.Element {
  ...
  const attachedSet = useAttachedSessions()
  const activeAttached = attachedSet.has(activeId ?? '')
  const activeTitle = sessions.find((s) => s.id === activeId)?.title || 'this session'
      ...
      {sessions.map((s, i) => {
        ...
        return (
          <button
            ...
            role="tab"
            aria-selected={selected}
            className={`isl-chip isl-chip-agent${selected ? ' active' : ''}${attachMode && attachedSet.has(s.id) ? ' attached' : ''}`}
          ...
          >
            ...
            <span className="isl-chip-dot" data-status={dotStatus(s.status)} aria-hidden />
            {attachMode && attachedSet.has(s.id) ? <span className="isl-chip-check" aria-hidden>✓</span> : null}
          </button>
        )
      })}
      </div>
  ...
  return (
    ...
    >
      ...
      {!openApp &&
        (attachMode ? (
          <div className="isl-attach-confirm">
            <button
              type="button"
              className={`isl-attach-check${activeAttached ? ' on' : ''}`}
              aria-pressed={activeAttached}
              disabled={!activeId}
              onClick={() => { if (activeId) toggleSession(activeId, activeTitle) }}
            >
              <span className="isl-attach-check-box" aria-hidden>
                {activeAttached ? (
                  <svg viewBox="0 0 24 24" aria-hidden focusable="false">
              ...
              <span className="isl-attach-check-label">{activeAttached ? 'Attached — tap to remove' : 'Attach this session'}</span>
            </button>
          </div>
        ) : (
